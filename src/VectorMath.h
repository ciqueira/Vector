...
static constexpr float kPi = 3.141592653589f;
static constexpr float kEpsilon = 1.0e-10f;
static constexpr float kOklabNeutralEpsilon = 2.0e-4f;
static constexpr float kOverlayReferenceHeight = 2160.0f;
static constexpr float kOverlayLineHalfThickness =
    5.0f / kOverlayReferenceHeight;
static constexpr float kOverlayLineFalloff = 1.0f / kOverlayReferenceHeight;
static constexpr float kOverlayPatchAtReference = 360.0f;
...
inline float circularDelta(float from, float to) {
  return std::fmod(to - from + 1.5f, 1.0f) - 0.5f;
}
...
inline float rgbOpponentHue(float3 rgb, float fallbackHue) {
  const float u = (2.0f * rgb.x - rgb.y - rgb.z) / std::sqrt(6.0f);
  const float v = (rgb.y - rgb.z) / std::sqrt(2.0f);
  if (std::sqrt(u * u + v * v) < 1.0e-7f) {
    return fallbackHue;
  }
  return wrapUnit(std::atan2(v, u) / (2.0f * kPi));
}

inline float stableBlueMask(float3 rgb, int inputCs) {
  if (inputCs == 0) {
    return 0.0f;
  }

  const float opponentHue = rgbOpponentHue(rgb, 2.0f / 3.0f);
  const float distance = std::fabs(circularDelta(2.0f / 3.0f, opponentHue));
  return 1.0f - smoothstep01(25.0f / 360.0f, 75.0f / 360.0f, distance);
}

inline float3 rgbToXyz(float3 rgb, int inputCs) {
  if (inputCs == 0) {
    return make_float3(rgb.x * 0.66245418f + rgb.y * 0.13400421f +
                           rgb.z * 0.15618766f,
  ...
  if (inputCs == 1) {
    return make_float3(rgb.x * 0.70062239f + rgb.y * 0.14877482f +
                           rgb.z * 0.10105872f,
  ...
  if (inputCs == 2) {
    return make_float3(rgb.x * 0.63800764f + rgb.y * 0.21470386f +
                           rgb.z * 0.09774445f,
  ...
  if (inputCs == 3) {
    return make_float3(rgb.x * 0.70485832f + rgb.y * 0.12976030f +
                           rgb.z * 0.11583731f,
...
inline float3 xyzToRgb(float3 xyz, int outputCs) {
  if (outputCs == 0) {
...
inline float3 adaptXyzD60ToD65(float3 xyz) {
  return make_float3(xyz.x * 0.987224008703f + xyz.y * -0.006113228607f +
...
inline float signedCubeRoot(float v) {
  return v < 0.0f ? -std::pow(-v, 1.0f / 3.0f)
                  : std::pow(v, 1.0f / 3.0f);
}

inline float3 xyzToOklab(float3 xyz) {
  const float3 lms = make_float3(
      ...
      xyz.x * 0.0482003018f + xyz.y * 0.2643662691f +
          xyz.z * 0.6338517070f);
  const float3 lmsPrime = make_float3(signedCubeRoot(lms.x),
                                      signedCubeRoot(lms.y),
                                      signedCubeRoot(lms.z));
  return make_float3(lmsPrime.x * 0.2104542553f +
...
inline float3 oklabToXyz(float3 oklab) {
  const float3 lmsPrime =
      make_float3(oklab.x + oklab.y * 0.3963377774f +
  ...
  const float3 lms = make_float3(lmsPrime.x * lmsPrime.x * lmsPrime.x,
                                 lmsPrime.y * lmsPrime.y * lmsPrime.y,
                                 lmsPrime.z * lmsPrime.z * lmsPrime.z);
  return make_float3(lms.x * 1.2270138511f + lms.y * -0.5577999807f +
                         lms.z * 0.2812561490f,
...
inline float3 neutralizeSmallOklabChroma(float3 oklab) {
  const float chroma = std::sqrt(oklab.y * oklab.y + oklab.z * oklab.z);
  const float threshold = kOklabNeutralEpsilon * std::max(1.0f, std::fabs(oklab.x));
  if (chroma <= threshold) {
    oklab.y = 0.0f;
    oklab.z = 0.0f;
  }
...
inline float3 oklabToOklch(float3 lab) {
  const float chroma = std::sqrt(lab.y * lab.y + lab.z * lab.z);
  float hue = std::atan2(lab.z, lab.y);
  if (hue < 0.0f) {
    hue += 2.0f * kPi;
  }
  return make_float3(hue / (2.0f * kPi), chroma, lab.x);
}

inline float3 oklchToOklab(float3 lch) {
  const float hue = lch.x * 2.0f * kPi;
  return make_float3(lch.z, lch.y * std::cos(hue),
                     lch.y * std::sin(hue));
}

inline float3 rgbToChen(float3 rgb) {
  const float rho = std::sqrt(rgb.x * rgb.x + rgb.y * rgb.y + rgb.z * rgb.z);
  if (rho < kEpsilon) {
  ...
  const float thetaDen = std::sqrt(std::max(0.0f, thetaDenSq));
  float theta = 0.0f;
  if (thetaDen >= kEpsilon) {
    theta = std::acos(clampf(thetaNum / thetaDen, -1.0f, 1.0f));
    if (rgb.y < rgb.z) {
      theta = 2.0f * kPi - theta;
    }
  }

  const float phiDen = std::sqrt(3.0f) * rho;
  const float phi = phiDen < kEpsilon
                        ? 0.0f
                        : std::acos(clampf((rgb.x + rgb.y + rgb.z) / phiDen,
                                           -1.0f, 1.0f));
  return make_float3(theta / (2.0f * kPi), phi, rho);
}

inline float3 chenToRgb(float3 chen) {
  const float theta = chen.x * 2.0f * kPi;
  ...
  return make_float3(
      rho * (0.81649658f * std::sin(phi) * std::cos(theta) +
             0.57735027f * std::cos(phi)),
      rho * (-0.40824829f * std::sin(phi) * std::cos(theta) +
             0.70710678f * std::sin(phi) * std::sin(theta) +
             0.57735027f * std::cos(phi)),
      rho * (-0.40824829f * std::sin(phi) * std::cos(theta) -
             0.70710678f * std::sin(phi) * std::sin(theta) +
             0.57735027f * std::cos(phi)));
...
inline float3 rgbToOklchModel(float3 rgb, int inputCs) {
...
inline float3 oklchModelToRgb(float3 oklch, int inputCs) {
...
inline float3 convertSaturationModel(float3 color, int model, bool toModel,
                                     int inputCs) {
  if (model == 1) {
...
inline int normalizedSaturationModel(int model) {
  return std::min(std::max(model, 0), 2);
}

inline float modelSatValue(float3 rgb, int model, int inputCs) {
...
inline float curveSatMultiplier(float satVal, const MCVectorParams &p) {
...
inline float3 applyModelSat(float3 rgb, float satMult, int model,
                            int inputCs) {
  model = normalizedSaturationModel(model);
  if (model == 0) {
  ...
  modelColor.y = std::max(0.0f, modelColor.y * satMult);
...
inline float3 applyCurvesSaturation(float3 in, const MCVectorParams &p) {
  ...
  const float satMult = curveSatMultiplier(modelSatValue(in, model, inputCs), p);
  ...
  if (model == 2) {
    ...
    if (blueMask > 0.0f) {
      ...
      out.x = mixf(out.x, sphericalOut.x, blueMask);
      out.y = mixf(out.y, sphericalOut.y, blueMask);
      out.z = mixf(out.z, sphericalOut.z, blueMask);
    }
  }
  return out;
}
...
inline float splitCurveLow(float curveBias) {
  return clampf(0.333f + splitCurveOffset(curveBias), 0.05f, 0.95f);
}

inline float splitCurveHigh(float curveBias) {
  return clampf(0.666f + splitCurveOffset(curveBias), 0.05f, 0.95f);
}

inline float bezierY(float t, float p1, float p2) {
  const float inv = 1.0f - t;
  return 3.0f * p1 * inv * inv * t + 3.0f * p2 * inv * t * t +
         t * t * t;
}

inline float smoothBell(float x, float center) {
  if (x <= 0.0f || x >= 1.0f) {
    return 0.0f;
  }
...
inline float shadowBellyCenterValue(float bellyCenter) {
  const float position = clampf(bellyCenter, -1.0f, 1.0f);
  return clampf(0.666f + position * 0.55f, 0.08f, 0.92f);
}

inline float shadowBiasedBezierY(float x, float curveBias, float bellyCenter) {
  const float offset = splitCurveOffset(curveBias);
  if (std::fabs(offset) <= 1.0e-6f) {
    return x;
  }
...
inline float shadowBellyEnvelope(float x, float bellyCenter) {
  const float position = clampf(bellyCenter, -1.0f, 1.0f);
  if (std::fabs(position) <= 1.0e-6f) {
...
inline float shadowBlackEnvelopeBase(float x) {
  const float inv = 1.0f - x;
  return inv * inv * inv + 3.0f * inv * inv * x;
}

inline float shadowBlackEnvelope(float x, float bellyCenter) {
  const float position = clampf(bellyCenter, -1.0f, 1.0f);
  if (std::fabs(position) <= 1.0e-6f) {
...
inline float3 applyVector(float3 in, const MCVectorParams &p) {
  ...
  return make_float3(base.x + (sat.x - base.x) + (zone.x - base.x),
                     base.y + (sat.y - base.y) + (zone.y - base.y),
                     base.z + (sat.z - base.z) + (zone.z - base.z));
}

inline float3 normalizePatchDelta(float3 delta) {
  const float minv = std::min(delta.x, std::min(delta.y, delta.z));
  float3 color = make_float3(delta.x - minv, delta.y - minv, delta.z - minv);
  ...
  return make_float3(color.x / maxv, color.y / maxv, color.z / maxv);
}
...
inline float encodeDavinciIntermediate(float x) {
  ...
  return x > linCut ? (std::log2(x + a) + b) * c : x * m;
}

inline float encodeAcescct(float x) {
  const float a = 10.5402377416545f;
  const float b = 0.0729055341958355f;
  const float c = 9.72f;
  const float d = 17.52f;
  const float e = 0.0078125f;
  return x <= e ? a * x + b : (std::log2(x) + c) / d;
}

inline float encodeLogC3(float x) {
  ...
  return x > cut ? c * std::log10(a * x + b) + d : e * x + f;
}

inline float encodeLogC4(float x) {
  const float a = (std::pow(2.0f, 18.0f) - 16.0f) / 117.45f;
  const float b = (1023.0f - 95.0f) / 1023.0f;
  const float c = 95.0f / 1023.0f;
  const float s =
      (7.0f * std::log(2.0f) * std::pow(2.0f, 7.0f - 14.0f * c / b)) /
      (a * b);
  const float t = (std::pow(2.0f, 14.0f * (-c / b) + 6.0f) - 64.0f) / a;
  return x < t ? (x - t) / s
               : (std::log2(a * x + 64.0f) - 6.0f) / 14.0f * b + c;
}

inline float3 encodePatchTransfer(float3 color, int pivotPreset) {
  color.x = std::max(0.0f, color.x);
  color.y = std::max(0.0f, color.y);
  color.z = std::max(0.0f, color.z);

  if (pivotPreset == 0) {
    return make_float3(encodeAcescct(color.x), encodeAcescct(color.y),
                       encodeAcescct(color.z));
  }
  if (pivotPreset == 1) {
    return make_float3(encodeDavinciIntermediate(color.x),
                       encodeDavinciIntermediate(color.y),
                       encodeDavinciIntermediate(color.z));
  }
  if (pivotPreset == 2) {
    return make_float3(encodeLogC3(color.x), encodeLogC3(color.y),
                       encodeLogC3(color.z));
  }
  if (pivotPreset == 3) {
    return make_float3(encodeLogC4(color.x), encodeLogC4(color.y),
                       encodeLogC4(color.z));
  }
  return color;
}
...
inline float3 getShadowPatchColor(float pivot, float colorMix,
  ...
  const float3 base = make_float3(sample, sample, sample);
  const float3 toned = applyShadowSplit(
      base, pivot, 1.0f, colorMix, neutralBlack, curveBias, redBellyCenter,
      greenBellyCenter, blueBellyCenter);
  return normalizePatchDelta(make_float3(toned.x - base.x, toned.y - base.y,
                                         toned.z - base.z));
}

inline float3 getHighlightPatchColor(float pivot, float colorMix,
                                     float neutralWhite, float curveBias) {
  if (pivot >= 1.0f) {
    return make_float3(0.82f, 0.82f, 0.82f);
  }

  const float sample = clampf(pivot + ((1.0f - pivot) * 0.5f), 0.0f, 1.0f);
  const float3 base = make_float3(sample, sample, sample);
  const float3 toned =
      applyHighlightSplit(base, pivot, 1.0f, colorMix, neutralWhite,
                          curveBias);
  return normalizePatchDelta(make_float3(toned.x - base.x, toned.y - base.y,
                                         toned.z - base.z));
}

inline float3 drawColorPatches(float3 out, int x, int y, int width, int height,
                               float3 shadowColor, float3 highlightColor,
                               float &overlayAlpha) {
  const float minDim = static_cast<float>(std::min(width, height));
  const float patch =
      kOverlayPatchAtReference * (minDim / kOverlayReferenceHeight);
  const float gap = patch * 0.12f;
  const float margin = minDim * 0.025f;
  const float left = static_cast<float>(width) - margin - patch;
  const float shadowTop = static_cast<float>(height) - margin - patch;
  const float highlightTop = shadowTop - gap - patch;
  const float xf = static_cast<float>(x);
  const float yf = static_cast<float>(height - 1 - y);
...
inline float lineAlpha(float curveY, float screenY, float halfThickness,
                       float falloff) {
  const float dist = std::fabs(curveY - screenY);
  return clampf(1.0f - (dist - halfThickness) / falloff, 0.0f, 1.0f);
}

inline float3 overlayRgbLine(float3 current, float curveY, float screenY,
                             float3 color, float halfThickness,
                             float falloff, float &overlayAlpha) {
...
inline float3 drawSatCurve(float3 out, int x, int y, int width, int height,
                           const MCVectorParams &p, float &overlayAlpha) {
  ...
  const float halfThickness = kOverlayLineHalfThickness;
  const float spacing = halfThickness * 2.0f;
  const float falloff = kOverlayLineFalloff;
...
inline float3 drawZoneCurve(float3 out, int x, int y, int width, int height,
  ...
  const float pivotTone = std::pow(clampf(pivotX, 0.0f, 1.0f), gamma);
  const float pivotSatMult = zoneSatMultiplier(pivotTone, p);
  const float pivotCurve =
      clampf(0.5f + (pivotSatMult - 1.0f) * 0.5f, 0.0f, 1.0f);
  const float barHalfWidth = kOverlayLineHalfThickness;
  const float barHalfHeight = kOverlayLineHalfThickness * 8.0f;
  if (std::fabs(xf - pivotX) <= barHalfWidth &&
      std::fabs(yf - pivotCurve) <= barHalfHeight) {
    overlayAlpha = 1.0f;
    return encodeOverlayColor(make_float3(0.72f, 0.72f, 0.72f),
                              p.pivotPreset);
  }
